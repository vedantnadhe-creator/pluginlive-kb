...
**Login OTP email — uses PluginLive platform mail, NOT Resend (2026-07-06):** login sends a real emailed OTP (`sendStudentLoginOtpEmail` in `src/lib/auth-email.functions.ts`). Upstream ships a **Resend** integration, but the Resend account is unverified/test-mode (owner-email only). Instead we route through the **platform's own SES-backed mail service** (same one user-management-node uses: `POST https://mail.prod.pluginlive.com/send-email` with an `auth-key` header, payload `{fromAddress,toAdresses:[],subject,html,text,messageType:"email"}`) — sends to any recipient, no domain-verification limit. Patched via **`~/ucat-ai-prep/reapply-pluginlive-mail.py`** (untracked, run after `reapply-gemini.sh`, before build): rewrites the send fn to use `PLUGINLIVE_MAIL_ENDPOINT`/`PLUGINLIVE_MAIL_AUTH_KEY`/`PLUGINLIVE_MAIL_SENDER` (added to `.env`; server-only, not in client bundle) and flips `getAuthEmailConfig` provider to "pluginlive". Verified: send to a real student email returns "Email successfully sent!"; server bundle has 0 `api.resend.com` refs. Reapply order now: reapply-gemini.sh → reapply-pluginlive-mail.py → force .env → build → reapply-nitro-node-shim.py → pm2 restart.
