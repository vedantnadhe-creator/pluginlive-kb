...
- `form-data-normalization` (candidate-data normalization LLM disambiguation, **plus the entity matcher** — institute / degree / city / state / country / role / department resolution) — DEV + UAT. **All three UAT containers (`datanormalization`, `-worker`, `-cron`) run the same `datanormalization:api` image and all three carry `LITELLM_*`, so the whole service is on the gateway** (verified 2026-08-18; the earlier note that the siblings ran a separate off-gateway image is no longer true). The deploy caveat still stands: `deploy.sh` recreates only the API container, so recreate the siblings by hand or they keep the old image — see `Infrastructure/form-data-normalization.md`.
- `pg-vector-api-service` (entity-normalizer LLM disambiguation/pincode) — code ready, gated; embeddings excluded.
- `resume-parser` (CV parsing — the `parseResume` / `parseResumeAndUpload` Gemini calls behind `form-data-normalization`'s CV ingest) — UAT (container `resumeparser`, port 5011). Routed via a tiny google-genai-compatible shim (`USING API/gateway_client.py`) using `requests`, not the OpenAI SDK. Gateway env passed at `docker run` (`-e LITELLM_PROXY_URL=http://172.17.0.1:4000/v1 -e LITELLM_VIRTUAL_KEY=…`), not baked into the image. ⚠️ **This bit us:** the container was recreated on 2026-07-18 without those flags and silently reverted to the raw `AQ.` key, so every UAT CV parse returned `500 / 401 UNAUTHENTICATED` for a month before anyone traced it. Re-applied 2026-08-19 — see `Infrastructure/document-parsers.md`. Any recreation of this container MUST re-pass both flags (and `OPENAI_API_KEY`, also not baked). Manual build/run (not in `auto_deploy`): `cd ~/api/Resume_parser/"USING API" && docker build -t resumeparser:api . && docker stop/rm + docker run`.
  ⚠️ **Superseded 2026-08-17** — CV *and* JD parsing now run **inside `fastapi-ai-engine`** (`/cv-parser/*`, `/jd-parser/*`) and therefore bill under the **`fastapi-ai-engine` virtual key**, not the `resume_parser` / `llama_jd_parser` keys. Those two keys still exist because the standalone services are still running, but `student-node` and `corporate-node` no longer call them. Per-parser spend now comes from the engine's usage ledger (`CV_Parser` / `JD_Parser` modules), not from the gateway's per-key view. See `Infrastructure/document-parsers.md`.
...
- **Root-level routes need an explicit mapping.** Module attribution comes from the router prefix, so anything mounted directly on the app has no module segment. `POST /validate-question` (Aptitude question validate-and-repair, ~$0.0025/call) billed as `Unattributed` until it was added to the exact-path table in `utils/ai_usage/context.py`. Add an entry there for any new root-level or legacy-aliased route that calls an LLM.
- **Which assessment types actually use an LLM.** Communication, Hinglish, Role_Based, AI_Interview and Aptitude do (generation, scoring, or both) and are tracked. **Custom_Assessment, Tech_MCQ, Tech_Coding, Behavior and Cognitive have no LLM scoring anywhere** — they are MCQ/auto-scored or handled by `code-runner`, so there is nothing to attribute and deliberately no module entry for them. Communication dictation scoring is likewise pure string comparison and correctly records nothing.
- **`AI_USAGE_DATABASE_URL` must be set per environment or nothing is recorded at all.** DEV ran with the ledger silently disabled until 2026-08-18 — the code was deployed but the variable was missing from its `.env`, so DEV showed zero rows while UAT and PROD recorded normally. If an environment reports no usage, check this variable before looking for a code bug.
...
- **`resume-parser` and `jdparser` bypass both sources** — they call Gemini directly with a raw key, so their cost appears only in Google Cloud billing. **As of 2026-08-17 this no longer applies to production traffic**: both parsers run inside `fastapi-ai-engine`, go through the gateway, and appear in the ledger as the `CV_Parser` / `JD_Parser` modules. Only calls made directly against the still-running standalone containers remain invisible.
