...
Meta re-approval is involved.

The over-36h branch was repaired on 2026-09-11 after a UAT invite exposed that
the implementation and its test had drifted to the raw map end date (for
example, `Complete it by 14 Sep 2026 11:28 AM`). It now follows the intended
36-hour rule above and is pinned by a regression test using that exact case.
The approved Meta bodies contain only `{{5}}` at that position, so this was a
backend parameter correction and required no template change or Meta
