...
- **Refresh debounce:** `pgRefreshMvs` skips a matview refreshed successfully within `SEARCH_MV_REFRESH_TTL_SECONDS` (default 900) — the 3-min crons would otherwise re-run each MV's full defining query (`mv_events_degree_stream_specialisations_master` alone burned ~45 min/day of UAT DB time). User-initiated syncs pass a short `ttlOverrideSeconds` instead: `/sync/institute_campus_cources` and `/sync/del_index_institute_campus_cources` use **30s**, so a course added in admin-react/institute-react is searchable in seconds rather than waiting out the 15-min timer window, while a bulk upload firing per row still coalesces. Those two endpoints also refresh **`mv_events_degree_stream_specialisations_master`** (built from `institute.courses`) — they are the only reliable 'courses changed' signal both frontends already send, so no per-call-site frontend hook is needed.
...
- **DB migrations** live in `PluginLive-Technologies/DB-Scripts` → `Search Service Postgres Migration/` (001–009 + `institute_job_role_mv` + `prod_missing_institute_views` + `stripped_name_expression_trgm_indexes`). DEV + UAT applied; **PROD: run any still-pending ones sorted by filename** — apply all of them (sorted by filename) before ever flipping PROD to `pg`.
