...
| UAT | **Postgres** (`SEARCH_ENGINE_DEFAULT=pg`) | Full PG code deployed 2026-07-10; flag lives in the box `.env` of container `searchservice` |
| PROD | **Postgres** (`SEARCH_ENGINE_DEFAULT=pg`) | Verified 2026-08-24 in ConfigMap `search-service-config`. All three envs are now PG — the ES sections below are legacy reference, not production truth |
...
- **Reads** are served from materialized views in the `search_engine` Postgres schema (one MV per former ES index, e.g. `mv_institutes_master`, `mv_corporate_list`, `mv_degree_stream_specialisation_master`, `mv_institute_job_role`). Query logic lives in `src/modules/search/pg/pgSearch.service.ts`. Search semantics mirror ES: per-word prefix AND for multi-word terms, punctuation/space-stripped matching (`B.C.S` ⇄ `BCS`, `tamilnadu` ⇄ `Tamil Nadu` — incl. the geo states/cities lookup), trigram typo tolerance for single words, and exact-name > exact-short-form > prefix > contains ranking (score terms are NULL-proofed — `similarity(NULL, q)` once poisoned the ordering).
...
- **Refresh debounce:** `pgRefreshMvs` skips a matview refreshed successfully within `SEARCH_MV_REFRESH_TTL_SECONDS` (default 900) — the 3-min crons would otherwise re-run each MV's full defining query (`mv_events_degree_stream_specialisations_master` alone burned ~45 min/day of UAT DB time). User-initiated syncs pass a short `ttlOverrideSeconds` instead: `/sync/institutes_master`, `/sync/institute_campus_cources` and `/sync/del_index_institute_campus_cources` use **30s**, so a college or course added in admin-react/institute-react is searchable in seconds rather than waiting out the 15-min timer window, while a bulk upload firing per row still coalesces. The two course endpoints also refresh **`mv_events_degree_stream_specialisations_master`** (built from `institute.courses`) — they are the only reliable 'courses changed' signal both frontends already send, so no per-call-site frontend hook is needed.
  - **Every sync endpoint must go through `pgRefreshMvs`, never `refresh_one` directly.** `syncInstitutesMaster` called `refresh_one` inline until 2026-08-24, which made it the only endpoint with *no* TTL — so each System Config save paid the full `mv_institutes_master` refresh. A PROD trace on 2026-08-24 shows it running twice, 11.2 s each, within three minutes of one college create. A few methods still bypass the helper (e.g. `fetchCollegeNameMaster`); they are cron-driven rather than in a save path, but the same fix applies if they ever land in one.
- **A refresh is not cheap — cost is proportional to the whole MV, not the change.** `REFRESH MATERIALIZED VIEW CONCURRENTLY` re-runs the defining query and diffs the full result set; there is no incremental mode. Measured on PROD 2026-08-24 (`search_engine.refresh_log`, warm): `mv_institutes_master` **11.2 s** (86,906 rows), `mv_institute_campus_course` **5.1 s** (126,365 rows), `mv_institute_campus_master` **7.6 s** (86,904 rows). Cold (the 12-hourly scheduled runs) the same three took **42.2 s / 21.5 s / 26.5 s**. `search_engine.refresh_log` is the place to look — it records `view_name`, `duration_ms`, `row_count` and `status` for every refresh.
...
- **DB migrations** live in `PluginLive-Technologies/DB-Scripts` → `Search Service Postgres Migration/` (001–009 + `institute_job_role_mv` + `prod_missing_institute_views` + `stripped_name_expression_trgm_indexes`). DEV + UAT applied, and PROD is now on `pg` so its migrations are applied too. Any *new* migration must still be run on all three, sorted by filename.
