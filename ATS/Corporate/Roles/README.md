...
- **Job-details mail to TPOs on publish (second, separate email):** Alongside the application-form mail above, the same publish handler sends every newly-floated college's POCs a **role-details** email (`RoleShareToTPOTemplate`) carrying the blue **"Job Details"** table — the same table `institute-react` mails to students on institute role creation. Key differences from the application-form mail, and the reasons for them:
  - **Not gated on `tallyFormUrl`.** This mail carries no form link, so a college whose Tally form hasn't been generated yet still gets the role details. Gating it on the form URL would silently skip exactly the colleges that need chasing.
  - **Not gated on `isEmailSent`.** That flag tracks the *application-form* mail; reusing it would either suppress this mail or, if flipped here, permanently starve a college of the form mail. Instead the scope is **"colleges published by this action"** — `roleLinks` rows minus `publishedIdSet` (the set loaded when the drawer opened). Republishing therefore never re-mails a college already on the role, and no new DB flag was needed.
  - **One mail per POC**, not one per college, so the `Hi {name},` greeting is personalised from `poc_details[].name` (falls back to *Sir/Madam*). Subject: `New Job Opportunity - {role} at {company}`.
  - Table rows come from `corporate-react/src/utils/jobDetailsTable.js` (`buildJobDetailsTable`) rendered through `components/DetailTable` — **both ported from `institute-react`**, which is safe because both apps read the same `GET /corporates/jobs/:roleId` payload. `DetailTable` branches on `typeof document`, so it only emits real HTML when rendered in the browser (which is where `renderToString` runs here) — rendering it in Node yields an escaped table.
  - `Application Deadline` is `moment.utc(endDate)` formatted `DD-MM-YYYY hh:mm A`, matching the table's own *Application closes at* row and the other role emails. See `Known Gotchas` on role date/TZ handling before changing this.
  - **Frontend-only change** — no backend, no schema, no new template row in `admin.event_notification_template` (unlike the institute-side `institute_jobrole_creation_email`, which is a DB template whose `{table}` placeholder is likewise filled in by the browser).
