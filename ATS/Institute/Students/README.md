...
- **Resume sub-entry ids:** the normalized payload's `resume.{workExperience,projects,courses,internships,awards}` items arrive without a per-item `id`. student-node's `sanitizeResumeForPersist` (shared by the create-full create + update paths) backfills a UI-style 8-char base36 id (matching the student-react resume drawers) on any item missing one — so ERP-created resume entries are addressable/editable like UI-created ones. Fill-only-when-missing, so UI payloads (which already carry ids) are untouched. Existing pre-fix ERP rows are not retroactively backfilled.
