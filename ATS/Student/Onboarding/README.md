...
- **Activation-link state boundary:** `/onboarding/activate/:studentId` must show Set Password for `currentState` `-1`, `0`, **and `1`**. Institute assessment candidates created by the add-to-existing flow arrive at state `1`; `SetPasswordPage` advances them to state `2` after a successful password reset. Only state `2+` should redirect to the auth login page. Fixed in `student-react` DEV `c6acdb7e` / UAT `35ec4884` on 2026-08-21.
...
- **Duplicated across frontends:** `Assessment-React` carries a byte-for-byte copy of this same `Onboarding/Components/ActivateAccount/CheckOtpPage` + `checkOtp` action (unused by any live route there as of 2026-07-01, but present in the bundle) — the OTP error-message fix above had to be applied to **both** `student-react` (the live path) and `Assessment-React` to stay consistent. Check both repos for any future change to this module.
