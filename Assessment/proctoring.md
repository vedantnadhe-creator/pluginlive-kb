...
- **Corporate candidate drawer** (`corporate-react-v2`, Proctoring tab, DEV + UAT 2026-09-17): the same gallery as admin's, but served by student-node `POST /students/assessments/proctoring/snapshots` (all sessions of an attempt, oldest first, presigned in-process, `pending` while any session's `is_valid` is null) merged per part by corporate-node into `/candidates/profile` → `proctoring.snapshots` + `snapshotStats`. See `ATS/Corporate/assessment-dashboard.md` → "Proctoring snapshot gallery".
