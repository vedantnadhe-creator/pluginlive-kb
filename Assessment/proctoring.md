...
`finalizeProctoringReport(assessmentAssignedId)` aggregates all events + snapshots into a `proctoring_reports` row: summary metrics, a denormalised timeline, an **integrity band** (`clean` ≥85 / `review` ≥60 / `high_concern`) and an **integrity score** (0–100, per-type capped penalties). Finalize is triggered on submit (`submitAssessment`) and after scoring for the ordinary assessment types. For AI Interview, completion now enqueues a durable finalizer (DEV + UAT, 2026-09-09) that waits for scoring, reading analysis and snapshot CV processing before the upsert; scoring and mixed-audio completion both re-enqueue it as recovery signals. This replaces the former fire-and-forget call that could run while reading/snapshots were pending, delete the partial report, and never retry—leaving both the dashboard and self-gating PDF with no proctoring section. Idempotent.

The snapshot cron also runs a bounded reconciliation sweep after its normal work (DEV + UAT, 2026-09-10; PROD pending). It finds ended proctoring sessions attached to completed assignments where `proctoring_logs.is_valid` is still null or the `proctoring_reports` row is missing, while excluding sessions that still contain pending snapshots (`face_detected IN (-1, -2)`). It then reuses the existing legacy validation and report-finalization paths. Candidates are isolated with `Promise.allSettled`, so one bad row cannot stall the batch; `PROCTORING_RECONCILE_BATCH_SIZE` controls the per-minute batch and defaults to 100. This repairs the failure mode where a cron/pod race permanently left the UI at **Evaluating** and caused the PDF's self-gated score and event table to disappear despite completed CV processing.
