...
When biometric verification is enabled, the shared v2 preflight asks the candidate to say any few words (the UI suggests **“Hello, hello, hello”**). Recording starts **immediately** (no lead-in pause) in the same preferred browser containers as assessment answers (WebM/Opus on desktop, MP4/AAC on WebKit). The clip is **at least 3 s** (`VOICE_SAMPLE_MIN_MS`); after that it stops ~600 ms after the candidate goes quiet, with a 6 s hard cap (`VOICE_SAMPLE_MAX_MS`) — `createSpeechEndDetector` in `src/lib/voicePhrase.ts` (time-domain RMS ≥ 0.02 = speech; needs ≥150 ms of speech before it can end, so a click can’t cut it off). The clip goes in parallel to `/proctoring/detect-audio` (base64, via `/api/assessment/verify/audio`) and to the `/api/assessment/ai-interview/stt` batch proxy. **Pass = detect-audio hears a voice AND the transcript has any recognized alphanumeric word** — the same rule as PROD. Typical pass ≈ 4.5 s (3 s clip + ~1.5 s checks). If either service is unreachable the row says the speech check is unavailable and passes on the live-track result rather than locking candidates out. History: PROD (`release-v1.39-hotfix-14`) still records a fixed 6 s after a 1.2 s lead (≈ 9 s). `35cb69a` (2026-09-24) briefly dropped detect-audio and the minimum; `d762cd4` (2026-09-25, DEV+UAT) set the current 3 s-minimum rule after a probe showed detect-audio verifies a 3 s clip exactly as it does a 6 s one. Gotcha: the fixed RMS threshold means a noisy room never reads as silence, so the clip runs to the 6 s cap. **Gotcha (fixed `ead145b`, 2026-09-25): ElevenLabs Scribe (STT primary on DEV+UAT since 2026-09-25) returns audio-event tags for non-speech — `[pause]`, `[static]`, `[ticking sound]`, `[outro jingle]` — instead of `""`, so a silent candidate in a noisy room passed the any-letter check. `hasRecognizedSpeech` now strips `[...]`/`(...)` tags before looking for a word.** Also note `detect-audio`'s `audio_detected` is a level check only (and it even rates fan hum/clicks as `is_human_voice`), so STT is the only real speech gate; a loud background conversation can still pass.
...
Current free-form microphone-readiness revisions: DEV `5229aee`, UAT `b545650`. Both builds ran in their own environment through `auto_deploy.sh`; the UAT bundle contained no DEV hostnames and a headless UAT invite-error load returned HTTP 200 with no page errors. No production rollout.
