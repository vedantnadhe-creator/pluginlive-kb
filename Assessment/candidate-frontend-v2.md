...
When biometric verification is enabled, the shared v2 preflight asks the candidate to say any few words (the UI suggests **“Hello, hello, hello”**). Recording starts immediately (no lead-in) in the same preferred browser containers as assessment answers (WebM/Opus on desktop, MP4/AAC on WebKit) and **stops ~600 ms after the candidate goes quiet** (`createSpeechEndDetector` in `src/lib/voicePhrase.ts`: time-domain RMS ≥ 0.02 = speech, needs ≥150 ms of speech before it can end so a click can’t cut it off; 6 s hard cap `VOICE_SAMPLE_MAX_MS`). There is **no minimum clip length**: the blob goes to the `/api/assessment/ai-interview/stt` batch proxy and the **only pass criterion is a transcript with any recognized alphanumeric word** — one “hello” (~1.4 s clip incl. the silence tail) passes. `/proctoring/detect-audio` is **no longer called** by the mic row (since 2026-09-24, commit `35cb69a`; it needs several seconds of speech and was failing short clips). Typical pass time ≈ 2.5 s, down from ≈ 9 s (1.2 s lead + fixed 6 s recording + STT). An empty/undecodable clip or a transcript with no words blocks Begin and offers a retry. If STT itself is unreachable, the row says the speech check is unavailable and passes on the live-track result rather than locking candidates out. Gotcha: the fixed RMS threshold means a noisy room never reads as silence, so the clip runs to the 6 s cap (old behaviour) — calibrate on the first ~200 ms if that becomes common.
