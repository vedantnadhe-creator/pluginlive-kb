...
| `Assessment-React` | `InviteStart` is generic; `InviteAssessmentRunner` dispatches by `assessment_type`; the OTP input is a fixed-width 48 px box grid (the earlier overflow bug); demo OTP code hint removed; "Back to Home" hard-redirects to `/assessment`. |
...
- **Frontend bundle DEV-leak (fixed 2026-06-15).** The invite *email link* was correct (`assessment.uat...`), but after landing, the invite OTP/AI-interview flow still bounced candidates to DEV. Cause: `Assessment-React/src/modules/Assessments/Partials/AIInterview/aiInterviewInviteAPI.js` read its axios base URLs via **member access** (`const API_URL = process.env.API_URL`) with hardcoded `|| 'https://api-auth.dev.pluginlive.com/'` / `|| 'https://api-std.dev.pluginlive.com/'` fallbacks. In this webpack build `EnvironmentPlugin` does **not** reliably inline member-access `process.env.X` for this chunk (the bare `process.env.API_URL` survived into the bundle → `undefined` at runtime → dev fallback fired), even though a clean UAT build with the correct `.env.uat` was used. Fix: switch to **named-key destructuring** `const { API_URL, STD_API_URL } = process.env` (the pattern in `src/utils/*Request.js`, which inlines correctly) and drop the dev fallbacks so a missing var fails loudly instead of silently hitting DEV. Verify any UAT/PROD frontend build with `docker exec <c> sh -c "grep -rho '[a-z-]*\.dev\.pluginlive\.com' /app/build | sort | uniq -c"` → must be empty. Frontend envs are baked at build time, so **always rebuild the FE on the target box from the target-env checkout; never ship a DEV-built bundle to UAT/PROD.**
