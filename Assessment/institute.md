...
Re-runs `getStudentListForAssessment` with `pageSize: 'NA'` (all rows, no pagination) and builds the sheet with ExcelJS. Base columns are Roll Number, Candidate Name, Email, Degree, Department, Progression Level, Assmt. Taken On, Taken / Sent, and Total Score, followed by type-specific columns and Proctoring. **Proctoring is the v2 integrity verdict since 2026-09-21 (student-node `63d06ccb` Development, UAT `b1e2a2d2`; PROD pending):** `helpers/proctoringVerdict.js` reads `proctoring_reports.integrity_band` per attempt — `clean` → `Good`, `review` / `high_concern` → `Bad`, `no_data` or no report → `-` — the same rule admin-node's `exportStudentData` prints, so a student reads the same in the TPO workbooks and the admin/corporate/one-time exports. Before this the column came from legacy `proctoring_logs.is_valid` and printed **`Bad` for any completed attempt with no log rows** (on UAT 39 clean-band attempts exported Bad and 75 review-band attempts exported Good). The Diagnosis sheet uses the same helper, collapsed per student across the pair (any Bad wins, then Good, still `-` under 2 attempts); the schedule Overview sheet and the Student-wise export carry no proctoring column (aggregates). The on-screen v1 candidate list shares `getStudentListForAssessment`, so it shows the same verdict. Roll Number comes from `student.students.uni_roll_no` and falls back to `-`. The combined schedule workbook also includes it in the Overview student list and Diagnosis sheet, so every student-level sheet uses the same college identifier as the one-time Assessment Results export.
...
**Scores in the workbooks are whole numbers, matching the PDF.** The dashboard APIs return scores at 2dp because the on-screen tables render them that way (`Math.round(score * 100) / 100` in `CandidateList`, `toFixed(2)` for NPS in `TpoStudentListTable`), but the PDF reports print integers (`Math.round` on every percentage in `Assessment.js`, `formatNumber … 0` in `aptitudeReport.html`) — so one attempt read 50.83 in Excel and 51 in the PDF (2026-09-01). `TpoDashBoard._roundScore()` now rounds every score cell in all four exports: `exportExcelOfStudentListForAssessment` (Total Score + Critical/Quantitative/Logical + Reading/Listening/Speaking/Writing), `exportExcelOfDiagnosisDataForASchedule` (section averages remain `%`-suffixed raw percentages; its `AVG SCORE` column is the raw pair average, not rounded — see below), `exportExcelOfTpoStudentList` (Comm./Aptitude NPS) and the `exportScheduleToExcel` Overview sheet (NPS). Non-numeric and missing values keep rendering `-`; a genuine `0` still prints `0`.

**Diagnosis roster score consistency (2026-09-15, DEV + UAT; PROD pending):** the legacy institute and admin Diagnosis tables now label the column `SCORE`, matching the other assessment result tables, and render the paired Diagnosis result's logarithmically curved NPS at 2dp without a percent sign. The backend reads the confirmed pair result from `progression_history` (`isSecondInPair = true`) and applies `curveNPS` at read time. It returns `npsScore` only after both Diagnosis papers are calculated; incomplete/absent pairs render `-`. The raw `totalAvgScore` remains available for section breakdown compatibility but is no longer the on-screen headline score. Sorting uses `npsScore` too.

**Diagnosis PDF needs both papers (2026-09-16, DEV + UAT; PROD pending):** in the Diagnosis roster's student drawer (`Partials/StudentReport/index.js`, opened with `isDiagnosis`), **Download Report** is disabled while the student has attempted only one of the two Diagnosis papers (`takenSent = 1/2`, i.e. only `assessmentAssignedId1` is set) and the hover tooltip reads "Only 1 diagnosis attempted". Gate lives in `Partials/StudentReport/reportDownloadGate.js` with a node test; identical in admin-react. See `Assessment/admin-frontend.md` → Diagnosis Report.

**Diagnosis workbook shows the raw average, not NPS (2026-09-16, DEV + UAT; PROD pending):** the Diagnosis sheet from `exportExcelOfDiagnosisDataForASchedule` (schedule workbook / Diagnosis export) was briefly switched to the curved `npsScore` under `SCORE` on 2026-09-15; product asked for the **average score** in the sheet instead. The column is now `AVG SCORE` and holds `totalAvgScore` — the plain mean of the two Diagnosis papers — written as-is (e.g. `73.6`, not `_roundScore`d), `-` when the pair is incomplete. The on-screen Diagnosis tables keep curved NPS under `SCORE`, so the grid and the workbook intentionally disagree for Diagnosis: screen = NPS, Excel = raw average. Regression: `test/diagnosisExcelExport.spec.js`.
