...
- **Section-transition dialog (2026-07-14, UAT `2d7d77b`)** — moving to a genuinely new section shows a ~1.5s dialog with the target **section's icon** (Reading/Listening/Speaking/Writing Ability) in a branded ring + "Taking you to the next section" + the section name (no emoji). Scoped to real section changes only — the multi-question writing sub-types (Dictation, Sentence Completion, Sentence Build) already `return` during intra-section question navigation, so it never fires mid-section. **Shown for all Communication assessments** — it was originally gated on `isCorporate`, but that flag is `undefined` at runtime across the corporate entry paths so the gate never passed; the gate was dropped (both components only ever render Communication). The `isCorporate` seeding was also fixed at source in `DirectAssessment` + `InviteAssessmentRunner` for other consumers.
