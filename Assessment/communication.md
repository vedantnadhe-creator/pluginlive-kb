...
- **Recording countdown + section-transition dialog — REVERTED on 2026-07-14 (UAT `e86971f`).** A 3…2…1 "Get ready to record" pre-recording overlay and a "Taking you to the next section" transition dialog were built (and ported to the Hinglish fork), then fully reverted at the client's request — all five commits `git revert`ed on Development and promoted to UAT, restoring the files to their pre-feature state. UAT/PROD have neither cue. Do not re-add without a fresh request.
- **⚠️ Separate Hinglish fork** — corporate Communication with `response_language` = Hindi/Hinglish renders through `Assessment-React/.../Hinglishassmt/assessment.js`, a near-duplicate of `Communicationassmt/assessment.js`. **Any student-facing Communication UI change must be applied to BOTH files** or it silently won't appear for Hinglish-language corporate takers.
