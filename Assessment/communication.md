...
- **Corporate section-transition loader (2026-07-14, UAT `9d52f3f`)** — for **corporate** Communication assessments only (`currentAssessment.isCorporate`), moving to a genuinely new section shows a ~1.2s "Taking you to the next section: <Section>" card (Reading/Listening/Speaking/Writing Ability). Scoped to real section changes only — the multi-question writing sub-types (Dictation, Sentence Completion, Sentence Build) already `return` during intra-section question navigation, so the loader never fires mid-section. Institute assessments advance instantly, unchanged.
