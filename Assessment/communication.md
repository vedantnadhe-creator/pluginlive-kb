...
- Select **Listening Audio Accent** (default `en-IN` Indian English; other options: `en-US`, `en-GB`, `en-AU`). Default keeps the existing shared set pool; any non-default accent generates a fresh set on the fly (voices: `en-IN`→`en-IN-Chirp-HD-O`, `en-US`→`en-US-Wavenet-F`, `en-GB`→`en-GB-Wavenet-F`, `en-AU`→`en-AU-Wavenet-C`, see `QuestionGeneration/Communication/audio_generation_google.py`). Applies to **both institute and corporate** creates.
- Dispatches `assignCommunicationAssessment` action to backend
> **Accent must survive the admin-react assign whitelist (fixed 2026-07-24).** `admin-react` `createAssessments` → `filterAssessmentData()` (`src/modules/Assessment/actions.js`) rebuilds each assign payload from a **field whitelist**; the `Communication` branch omitted `accent`, so the admin's selection was silently dropped before `POST /assessment/assignAssessment`. Backend then defaulted to `en-IN` and **pool-picked an Indian set instead of generating the selected accent** — every non-`en-IN` corporate/institute assign came out Indian (symptom: US-accent assessment plays `en-IN-Chirp-HD-O`). Fix: added `accent: assessment.accent || 'en-IN'` to that branch. Backend + async queue were already accent-correct (`assignCommunicationAssessmentAsync` materializes a fresh non-default-accent set up front and binds it directly, skipping the pool-picker — `admin-node/app/models/Assessment.js:~6019-6035,6164`). Deployed DEV + UAT.
