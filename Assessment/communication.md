...
- **Recording countdown (2026-07-14, UAT `9d52f3f`)** — before recording starts on **Video Response** or **Paragraph Reading**, a full-screen 3…2…1 overlay ("Get ready to record") shows over the assessment content, gated via a small `startWithCountdown()` wrapper around the original `startRecording`/`startAudioRecording` functions (which are otherwise unchanged).
- **Corporate section-transition dialog (2026-07-14, UAT `cd49112`)** — for **corporate** Communication assessments only, moving to a genuinely new section shows a ~1.5s dialog with the target **section's icon** (Reading/Listening/Speaking/Writing Ability) in a branded ring + "Taking you to the next section" + the section name. Scoped to real section changes only — the multi-question writing sub-types (Dictation, Sentence Completion, Sentence Build) already `return` during intra-section question navigation, so it never fires mid-section. Institute assessments advance instantly, unchanged. **Corporate detection is resilient** (currentAssessment / assessment prop / assessmentData / `studentCorporateStatus`) because several entry paths (portal `DirectAssessment`, OTP-invite `InviteAssessmentRunner`) historically seeded `SET_CURRENT_ASSESSMENT_ID` without `isCorporate` (now also fixed at source).
- **⚠️ Separate Hinglish fork** — corporate Communication with `response_language` = Hindi/Hinglish renders through `Assessment-React/.../Hinglishassmt/assessment.js`, a near-duplicate of `Communicationassmt/assessment.js`. **Any student-facing Communication UI change must be applied to BOTH files** or it silently won't appear for Hinglish-language corporate takers. The recording countdown and section-transition dialog were ported to the Hinglish fork on 2026-07-14 for exactly this reason.
