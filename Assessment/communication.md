...
> **Third gate — admin-react-v2's label→code map (fixed 2026-09-02, DEV + UAT + PROD).** The v2 wizard's accent control is a `RadioCardGroup` whose **value is the label**, and its catalogue was narrowed to `["US", "Indian"]` (`src/lib/dummy/assessmentConfigOptions.ts`) while `communicationAccentCode` (`src/lib/assessments/communicationAccent.ts`) still only knew `"American"`. `"US"` was not a key, so the trailing `?? "en-IN"` swallowed it: `/api/assessments/mix-match` posted `accent: "en-IN"`, admin-node took the pool-pick path (`kind: "communication-main"` instead of `communication-generate`), and **every US float from the v2 admin since the narrowing shipped Indian-accented listening audio** — with no error anywhere. The Google voice was never involved: `en-US-Chirp3-HD-Kore` synthesizes fine on PROD (probed live against the `fast-api` pod, GCP project `pluginalex`). Diagnostic that isolates this: `assessment_assignment_jobs.config_snapshot.prepare.specs.main.accent` — a v2 float (`config_snapshot ? 'mixMatchGroupId'` is true) always read `en-IN`, while every genuine `en-US` job in both UAT and PROD came from the **v1** admin. Fix: map `US` alongside `American`, and **throw on an unknown non-empty label** instead of defaulting, so the next catalogue/map drift fails in front of the admin. **Lesson (third time on the same field):** accent has now been dropped by a v1 whitelist, a Fastify schema, and a v2 label map — any new field must be traced end-to-end from the control's value to `config_snapshot`, and a silent `?? default` on a user-chosen value is the bug, not the safety net.
>
...
> **Diagnosis papers never have their CEFR set swapped either (fixed 2026-09-16; DEV + UAT, PROD pending).** The delivery-time swap above only exempted one-time papers, so a diagnosis pair (`assessment_assigned_students.is_diagnosis = true`) was re-pitched whenever the student already carried a CEFR — the **profile fallback** fires for any student whose `student_personal_profile.assessment_cefr` is set, and that value is seeded by paths unrelated to the schedule: a **corporate** assign (`admin-node` creates corporate candidates with `AssessmentCEFR = <that assessment's level>`), add-students on a one-time paper, an older chain at another institute. Symptom (PROD, Christ University Lavasa, schedule `ad8510f7…` configured **B2**): 73 of 82 students sat B2 diagnosis papers, 9 sat A1/A2/B1 — none of the 9 had any prior Communication attempt, but e.g. `harsh.hajarnis@…` had been created on 2026-04-22 by a Revanta Capital corporate Communication assign at **B1**, so their Christ diagnosis opened at B1. The drill-in "Difficulty" column shows the *served* set level, so it correctly displayed a different level per student — the display was right, the delivery was wrong. Fix: `isAdaptiveDelivery({ isInstitute, isOneTime, isDiagnosis })` (`student-node/app/helpers/communicationSetSwap.js`) now gates both the Communication swaps **and** the Aptitude progression-difficulty overwrite (`getAptitudeAssessmentQuestions`); only a recurring institute paper adapts. No backfill — existing diagnosis results stay as sat. Rule: **every student in a schedule sits their diagnosis at the schedule's configured level**; per-student variation of the *paper* (different set ids at the same level) is still by design.
