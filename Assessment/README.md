...
- `behaviour.md` -- Behaviour Assessment (competency proficiency levels + suitable job roles). Role matching is by competency **name** across the whole catalogue, proportionally scored -- requirements are almost all Expert/Master, so pass/fail matching returns nothing. Candidate paper is flattened + seed-shuffled with every question mandatory (client-side by design); admin wizard quotes the real bank (115/129) from admin-node. Live DEV + UAT.
