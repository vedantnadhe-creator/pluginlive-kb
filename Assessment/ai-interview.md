...
| `POST /ai-interview/tts` | **Text-to-speech** — ElevenLabs Payal/Anika (Flash v2.5); auto-falls-back to Deepgram Aura-2 (`aura-2-thalia-en`). Body: `{ text, voice?, language? }`; `language=Hindi` adds ElevenLabs `language_code=hi`. Returns audio bytes (mp3). |
