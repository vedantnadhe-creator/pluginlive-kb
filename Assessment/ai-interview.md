...
**ROLLBACK PLAN for the 2026-08-03 Hinglish work (tested 2026-08-03).** The three Hinglish prompt commits are `0eb9f5a` (2026-07-30, pure-Hindi fix), `cfad015` (clause-level mix), `8e83d79` (TTS spelling). **Reverting today's two is clean and is the supported rollback:** `git revert --no-edit 8e83d79 cfad015` — verified to apply without conflict, compile, leave the mid-interview language-switch feature fully intact, and restore the 2026-07-30 behaviour. **Reverting `0eb9f5a` as well CONFLICTS** and must not be attempted casually: the language-switch feature (`ec27902`, 2026-07-31) was built on top of it and calls `_language_rules()`, so removing it breaks that feature — it would have to be reverted or reworked too. UAT rollback point for the pre-2026-08-03 state is `81c12ee`; the pre-clause-fix state is `a850545`. **Note:** reverting restores the *previous* defects — `cfad015` fixed "Hindi grammar with English nouns" and `8e83d79` fixed the `use`/उसे mispronunciation, so a full revert brings both back.
