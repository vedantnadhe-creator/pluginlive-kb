...
- **⚠ Two open v2 bugs, neither fixed (2026-09-23).** (a) **A reframed question is never spoken.** The speak effect is keyed `[camera, askTicket, liveQuestion?.interactionId]`; a reframe returns the same `interactionId` and does not bump `askTicket`, so the effect never re-runs — the candidate sees reworded text and hears silence. Pre-existing and independent of the progress work. One-line fix: bump `askTicket` on the reframe branch, the same mechanism the silent re-ask already uses. (b) **The DEV build of `candidate-assessment-journey-v2` is broken by untracked work-in-progress in the shared checkout.** `.dockerignore` excludes `node_modules`/`.next`/`.git`/env files but **not untracked sources**, and Next type-checks the whole `src` tree, so `src/lib/proctoringEnvironment.ts` (untracked, emits `"split_screen"` which is absent from the committed `ProctoringEventType` union) fails the build: *Type error: Argument of type '"split_screen"' is not assignable*. Nothing imports it; it fails anyway. `auto_deploy.sh` left the old container running, so DEV is serving fine — it is simply stuck on the previous build until that file is committed or removed. UAT's checkout is clean, which is why UAT deployed normally.
