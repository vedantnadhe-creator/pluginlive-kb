...
| **TTS (interviewer voice)** | **ElevenLabs Flash v2.5** (`eleven_flash_v2_5`) — admin-selectable **Payal** (default; conversational) or **Anika** (lively), both professional Hindi voices. Candidate v2 sends the hidden Devanagari `speechText` plus the stored per-question language; Hindi turns set ElevenLabs `language_code=hi`, while English turns using the same voice remain English. Delivery style is `0.15` for calmer interviewer speech. **Fallback chain (2026-09-12, DEV + UAT; PROD pending):** ElevenLabs → **Sarvam Bulbul v3** (`bulbul:v3`, speaker `ritu`, `hi-IN`/`en-IN`/10 Indic codes, mp3) → Deepgram Aura-2 `aura-2-thalia-en` only for languages Bulbul cannot speak (Urdu, Assamese). Deepgram's TTS catalogue has **no Hindi and no en-IN** — fed Devanagari it produces gibberish ("main APCA interviewer who"), which is what candidates heard whenever the ElevenLabs quota ran out. English turns also go to Sarvam so a Hinglish interview keeps one interviewer voice. An ElevenLabs 401/402/429 stands it down for 10 min (`ELEVENLABS_COOLDOWN_SECONDS`) instead of probing on every line; it resumes automatically once the quota is back. Overridable via `ELEVENLABS_VOICE_ID` / `ELEVENLABS_MODEL_ID` / `SARVAM_TTS_SPEAKER` env vars on the FastAPI container. Sarvam TTS is metered in the AI-usage ledger as `sarvam_tts` (Rs 15/10K chars). |
