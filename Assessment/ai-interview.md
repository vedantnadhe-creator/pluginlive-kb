...
- **⚠ The UAT deploy of this is BACKEND-ONLY and therefore inert (2026-09-22).** `fastapi-ai-engine` (`7b1e5e8`) and `student-node` (`851f9ec8`) are on UAT and verified — a Kannada line returns byte-identical audio to a direct Sarvam call. But **`Assessment-React` was deliberately NOT promoted**: its `Development` carries five unrelated commits UAT does not have, including `18bb2ec feat(auth): keep the session alive with the new refresh-token flow` — one half of an 11-repo feature that is DEV-only and whose UMS counterpart (`POST /user/token/refresh`, the `user_management.refresh_tokens` table, `LOGIN_TOKEN_EXPIRES_IN=2d`) is not deployed on UAT. Merging would install refresh interceptors in the candidate app pointing at an endpoint UAT does not serve. Because the candidate app is what sends `language`/`languages`, **a real UAT interview still routes every language to ElevenLabs Flash and regional audio is still wrong there**. The backend change is safe and behaviour-preserving without it (no `languages` → plan defaults to `eleven_flash`), just not yet useful. To finish: either promote the auth work to UAT with its DB + env prerequisites first, or land the two TTS-only commits on UAT through a branch that excludes the auth commit.
