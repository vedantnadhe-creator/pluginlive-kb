...
- **Per-question language state hardens dynamic generation (updated 2026-09-11).** Each generated interaction stores `questionMetadata.questionLanguage`. Student-node sends the ordered language history and current per-language counts on every subsequent generation call, including priority/must-ask questions, so the model can follow unified instructions such as alternating languages or placing one language later without relying on a stateless prompt. An explicit two-language percentage instruction is converted into whole-question targets against the complete configured question budget, including the introduction (for example, 80/20 over eight questions becomes six/two), with simple `last` ordering respected. **Default split (added 2026-09-16):** when a two-language interview (primary + secondary, or Hinglish) has no percentage in its instructions and the instructions name neither language, student-node applies `DEFAULT_LANGUAGE_SPLIT = [70, 30]` with the secondary (Hindi for Hinglish) asked as the final block — eight questions become six primary then two secondary, and the introduction is always in the primary. Free-form instructions that do name a language (e.g. "ask the scenarios in Marathi") are still left to the model. Precedence: explicit `NN%` → free-form naming a language → default 70/30-last. FastAPI's `RETRY OVERRIDE` now states that the forced per-turn language overrides the base LANGUAGE INSTRUCTION for that turn, and the Hinglish rule defers to the QUESTION LANGUAGE STATE block instead of asking the model to balance turns. FastAPI returns an explicit `question_language`, validates the generated display/speech text against the required or selected configured language, and regenerates a mismatch once. If the constrained retry still fails validation, FastAPI logs the exact failed rule names and serves the sanitized non-empty retry with transition metadata removed rather than returning a mid-interview 502. Every question starts directly without a language transition sentence, acknowledgement, recap, or scene-setting preamble. The first hesitation records a language-adherence mismatch and re-asks a simplified version in the same language. The second hesitation for the same target records another mismatch and permanently switches remaining questions to the other configured language (one switch per session), so the interview can continue without stopping the penalty. Since 2026-09-16 an answer given in the *other configured language* is a hesitation signal too — see the next bullet. No switch can leave the configured primary/secondary boundary. A Hinglish-to-Hindi hesitation switch remains in Hinglish generation mode so the candidate sees conversational Roman Hindi while TTS receives the separate Devanagari speech text. Candidate UI changes must deploy through the v2 target `assessment-react-v2` / `candidate-assessment-journey-v2`; the legacy `assessment-react` target does not serve this journey. DEV + UAT live; PROD pending.
- **Answers must follow the question language (updated 2026-09-11).** The per-turn scorer receives the stored `questionLanguage`, classifies the answer's predominant language, and ignores necessary technical terms, names, brands, and brief borrowed words. An uncertain/very short answer is not treated as a mismatch. Primary/secondary configurations (updated 2026-09-16): an answer whose predominant language is the **other configured language** (an English question answered in Tamil when Tamil is the secondary) is treated as a language signal, not a wrong answer — it runs through the same two-signal state machine as an explicit hesitation, sharing its `sessionMetadata.languageHesitation` counter. First signal: the same question is re-asked with the spoken/shown nudge “Please re-answer in [question language].” (no LLM call; the persisted `questionText` stays bare so a later switch re-asks the question, not the nudge), the attempt is recorded under `questionMetadata.languageMismatchAttempts` with `reason: "off_language_answer"`, and `languageMismatchCount` is incremented. Second signal for the same language (another such answer **or** an explicit "I can't follow English"): the interview switches to that language for all remaining questions, the candidate app pins STT/TTS to it, and `languageMismatchCount` increments again — the penalty mirrors hesitation exactly (2 signals ⇒ 80% multiplier). Before this change every such answer was bounced with “Please re-answer in English” indefinitely, because the check compared against the question's language rather than the configured pair (found on UAT 2026-09-16, English+Tamil). An answer in a language **outside** the configured pair, or any off-language answer after the single allowed switch, still gets the strict re-answer bounce with a mismatch each time. Single-language interviews are unchanged: strict bounce, nothing to switch to. **Hinglish is the exception:** questions remain isolated English or conversational Roman Hindi, but candidates may answer naturally in English, Hindi, or mixed Hinglish without a language retry or mismatch penalty. Explicit difficulty phrases such as “I don't know Hindi” or “switch to English” are still detected before ordinary mismatch handling and follow the same two-step hesitation flow: simplify/re-ask on the first consecutive occurrence, then switch within the configured boundary on the second. Strict-config language adherence uses one multiplier schedule: 0 mismatches = 100%, 1 = 90%, 2 = 80%, and 3+ = 70%. The multiplier adjusts the transcript-derived Language Proficiency sub-score (and therefore its stars and the combined Communication & Language score) and also applies to the final overall score; there is no separate fixed 15-point deduction. The report groups mismatches in one line, such as `English expected → Hindi detected (2×)`. Other role-parameter evidence remains unaffected. DEV + UAT live; PROD pending.
