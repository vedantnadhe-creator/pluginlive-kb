...
- **AI Interview scores** (fixed 2026-08-11): the endpoint scores per assessment type, and until this fix it had branches for Aptitude / Communication / Behavior / Role_Based / Custom but **none for `AI_Interview`** — so every AI interview candidate returned `totalScore: null` and the Total Score column rendered `-` even when a finalized score existed. Now:
  - `ai_interview_scores` has **no Prisma relation back to the assignment** (the graph is `assessment_assigned_students` → `ai_interview_sessions` → `ai_interview_scores`), so the rows are fetched in one batched query by `assessment_assigned_id` for the page's assignments and indexed by `app/helpers/aiInterviewDashboardScore.js` (`indexAiInterviewScores`, latest row wins). No N+1.
  - `totalScore` = `overall_score` (already a 0–100 value), rounded to 2 dp. Sorting by score works because the sort pass now also selects the assignment `id`.
...
- **Export**: reuses admin-node `/assessment/exportStudentData` with `entityType=corporate` (corporate-react-v2 sends `POST` with `{selectedEmails}` in the JSON body since 2026-09-11 so bulk selections never sit in the URL; the legacy `GET` query form is still accepted — see `ai-interview.md` § Corporate Export Sheet) (the same Excel builder as institute, so it includes the role-based section columns incl. Coding). One selected status exports that bucket; otherwise exports all (`status=sent`). The **Roll Number** column is the one column that differs: it is gated on `entityType === "college"`, so it appears in the institute export only.
