...
Storage reuses `admin.feature_config`: the active `CORPORATE / ASSESSMENT_CREATION_DISABLED` row contains the opted-out corporate IDs in `journey_ids`. The row is created lazily with an atomic upsert on the existing partial unique index; enabling removes that corporate ID. No schema migration or backfill is needed. An absent opt-out means enabled; database errors propagate instead of granting access. Reads are uncached, so each submission checks the latest saved setting.

In **corporate-react-v2**, `/api/me/feature-access` scopes the lookup to the forwarded token's corporate. Disabled corporates lose Create assessment / Float Another Assessment actions, and direct navigation to the wizard displays an access message. The `/api/assessments/mix-match` submission handler rechecks access before creating sections, groups, schedules or invites: disabled returns **403**, unavailable access returns **502**. An already-open wizard therefore cannot bypass a subsequent disable. Existing assessment viewing remains available. Deploy admin-node before either frontend because both require the new response flag; an older upstream response is treated as unavailable.
