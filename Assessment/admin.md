...
**Columns are per assessment type.** A fixed base block (**Roll Number** — college only, Name, Email, Phone, **Degree, Department**, Sent Date, Start Date, End Date, Status, Delivery Status, Delivery Issue) is followed by type-specific score columns, then Proctoring Status when proctoring is enabled. Degree and Department use the same current-course-first values returned by `getAssessmentDetails`, falling back to the education profile and then `"-"`. **Degree and Department are college-only columns.** They were briefly added to corporate workbooks on 2026-09-16, but product asked for them to go (a floated candidate has no course data, so the columns were all `"-"`); **since 2026-09-18 (DEV+UAT, PROD pending) the corporate branch (`entityType === "corporate"`) omits both columns** while college exports are unchanged. The corporate v2 export (`corporate-node /corporates/:id/assessments/v2/:id/candidates/export` → admin-node `POST /assessment/exportStudentData`) shares this code path, which is why the gate lives here and not in corporate-node.
...
**Session Name and ID were removed (September 2026).** Session Name repeated the assessment's own name on every row and the name is already in the filename; ID was the internal student UUID, which nobody reading the sheet uses. Dropping both left the sheet starting at the identifier that matters. Both the college and corporate exports lost these two columns; only the college export gains a leading Roll Number (corporate has none, so its sheet now starts at Name).
...
**Corporate assessment creation toggle (DEV + UAT, 2026-09-16)** — Feature Access now includes **Allow assessment creation**, enabled by default for existing and new corporates. `POST /assessment/assignSubscription` accepts the optional boolean `assessmentCreationEnabled`; omission preserves the saved setting and institute saves do not change it. `GET /assessment/featureAccess` returns `ASSESSMENT_CREATION` alongside the WhatsApp flag. The admin form loads the selected corporate's flag, prevents stale responses when changing entities, and blocks saving if access could not be loaded.

Storage reuses `admin.feature_config`: the active `CORPORATE / ASSESSMENT_CREATION_DISABLED` row contains the opted-out corporate IDs in `journey_ids`. The row is created lazily with an atomic upsert on the existing partial unique index; enabling removes that corporate ID. No schema migration or backfill is needed. An absent opt-out means enabled; database errors propagate to the UI access lookup. Reads are uncached.

In **corporate-react-v2**, `/api/me/feature-access` scopes the lookup to the forwarded token's corporate. Disabled corporates lose Create assessment / Float Another Assessment actions, and direct navigation to the wizard displays an access message. This toggle is **UI-only**, per the 2026-09-16 correction: `/api/assessments/mix-match` does not read or enforce it and does not reject submissions because it is disabled. Existing sign-in, subscription and payload checks still apply. Existing assessment viewing remains available. Deploy admin-node before either frontend because both require the new response flag; an older upstream response is treated as unavailable.

**Corporate Manage assessment toggle (DEV + UAT, 2026-09-16)** — Corporate Feature Access also has **Allow assessment management**, default ON and independent of creation access. The optional `assessmentManagementEnabled` boolean on `POST /assessment/assignSubscription` persists the UI setting; omitted values preserve it and institute saves ignore it. The existing `admin.feature_config` store uses a separate `CORPORATE / ASSESSMENT_MANAGEMENT_DISABLED` opt-out row, created lazily, so no schema migration is required. `GET /assessment/featureAccess` exposes `ASSESSMENT_MANAGEMENT`; corporate-react-v2 `/api/me/feature-access` returns `canManageAssessment` alongside `canCreateAssessment`.

OFF hides only the **Manage assessment** gear and prevents its settings drawer from opening on the corporate assessment detail page (including closed-assessment configuration). Lists, results, Share, Add candidates and Reopen remain available. This is **UI-only**: no assessment mutation endpoint enforces the management toggle. The admin form will not save a corporate's settings until both flags load successfully. Deploy admin-node before the two frontends.
...
Corporate v2 requires its visible validity input and sends exactly what the
recruiter entered; it has no hidden payload fallback. The duplicated stale
Admin v2 creation view still defaults a missing value to 3 days so it cannot
fail the float with `assessmentValidityDays must be a positive integer`.
Explicitly supplied invalid values are rejected. Manage Assessment reads and updates it,
