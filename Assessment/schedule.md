...
- **Full Schedule renders stored wall-clock time without a second timezone shift (fixed 2026-09-11; DEV + UAT)** — `institute-react-v2` schedule cards receive assessment map timestamps whose UTC-shaped value already contains the intended IST wall-clock digits. `EventRow` previously used `new Date(start).toLocaleTimeString(...)` with the browser's local timezone, so an intended `00:00` appeared as `05:30` on an India device. Schedule cards now use `formatScheduleTime` from `src/lib/schedule/dates.ts`, which formats with `timeZone: "UTC"` and therefore preserves the stored wall-clock value. Regression coverage verifies that midnight renders as `12:00 am` under an IST process timezone. Commit `54addd7`.
...
- **The attempt-start gate enforces the window at the IST deadline (2026-08-31)** — `getAssessmentQuestions` (`student-node/app/handlers/assessmentHandler.js`) now re-reads the attached map's `start_time`/`end_time` at the hard start boundary and refuses a start outside it with **410 `ASSESSMENT_WINDOW_CLOSED`**. Previously the dashboard filter was the *only* thing keeping a candidate out of a closed assessment, so a stale open tab or a direct request could start one indefinitely after its deadline. The comparison goes through the shared helper `student-node/app/helpers/assessmentWindow.js` (`wallClockIstToInstant` / `isAssessmentWindowOpen`), which subtracts the 5:30 offset because these columns hold **IST wall-clock digits in a UTC-shaped value** — without it the gate would itself have closed 5.5h late. `is_practice` assignments are exempt (practice has no map window), and an unbounded side of the window is treated as open. Covered by `student-node/test/assessmentWindow.spec.js`. On the candidate side, `Assessment-React` `modules/Assessments/actions.js` routes **410** through the same terminal branch the 409 single-active-attempt guard already used, so the candidate gets an "Assessment unavailable" modal and the stale in-progress marker is cleared — without that the assessment correctly refused to start but nothing visible happened, because `questionsError` is not rendered on every instruction screen. DEV + UAT 2026-08-31; **PROD pending**.
