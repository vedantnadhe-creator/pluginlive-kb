...
**On a phone, neither event is punitive (2026-08-26, `078241a`).** An incoming call backgrounds the page, and the browser reports that identically to the candidate deliberately switching apps — so three calls used to spend all three warnings and auto-submit the paper. `watchVisibility` and the `fullscreenTransition` exit in `src/app/assessment/take/page.tsx` now call `markMobileInterruption()` / `isMobileInterruptionFullscreenExit()` (`src/lib/deviceTier.ts`) first: on a phone or tablet both events are still **recorded** — at `severity: "low"`, `confidence: 0.25`, so a reviewer can weigh them — but neither increments `MAX_TAB_VIOLATIONS`. The fullscreen guard has a 3s grace window (`INTERRUPTION_GRACE_MS`) because mobile browsers drop fullscreen a beat *after* the visibility change, and one call must not cost two warnings. Desktop is unchanged. `proctoringEvent` gained a `confidence` option for this; it was hardcoded to `1`. Live DEV + UAT.
