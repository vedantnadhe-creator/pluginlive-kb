...
**On a phone the visibility change is not punitive; the fullscreen exit always is (2026-08-26, `078241a` then `17bfa86`).** An incoming call backgrounds the page, and the browser reports that identically to the candidate deliberately switching apps — so three calls used to spend all three warnings and auto-submit the paper. The two halves are now treated differently, and the split is deliberate:

| Event | Mobile | Desktop |
|---|---|---|
| `visibilitychange` → hidden | recorded at `severity: "low"`, `confidence: 0.25`; **no violation** | recorded `high`; violation |
| fullscreen exit | recorded `medium`; **violation** | recorded `medium`; violation |

`watchVisibility` in `src/app/assessment/take/page.tsx` checks `isMobileDevice()` (`src/lib/deviceTier.ts`) and skips the `TAB_VIOLATION` dispatch; the `fullscreenTransition` exit dispatches unconditionally. `proctoringEvent` gained a `confidence` option for the low-confidence signal; it was hardcoded to `1`.

The first cut (`078241a`) exempted **both** events on mobile, and that was wrong: a phone reports a deliberate app switch as a fullscreen exit *and nothing else*, so switching away mid-exam cost nothing at all. `17bfa86` made the fullscreen exit unconditional and deleted the grace window and interruption timestamp that existed only to suppress it.

**Known trade-off, accepted:** a call that drops full screen now costs one violation (it used to cost two before any of this, and zero between the two commits). No browser API distinguishes a call from an app switch, so the choice is which error to make; leaving the switch free was the worse one. Live DEV + UAT.
...
| Microphone | `POST /proctoring/detect-audio` + `POST /api/assessment/ai-interview/stt` | `audio_detected` AND transcript has any recognized word |

Since 2026-09-25 (DEV+UAT) the mic row records from the moment "Say a few words…" shows, for at least 3 s, then until ~600 ms after the candidate goes quiet (6 s cap). PROD still records a fixed 6 s after a 1.2 s lead. See `candidate-frontend-v2.md` → voice readiness.
...
- **Voice** — `POST /proctoring/detect-audio` with `{student_id, audio_data}` → `audio_detected`, on a clip of at least 3 s (verified equivalent to 6 s), combined with batch STT returning any word.
