...
> **Single-shared-stream was tried and reverted (do not reintroduce naively).** An attempt to run proctoring **and** video-response recording off one persistent `video+audio` stream (test page proved the model) **broke recording on UAT** — "camera permission" / `NotReadableError` and lost takes. Cause: in the assessment runtimes the proctor stream is torn down in **~7 places** (the `[stream]`/`[isFullPageMode]` cleanup effect that calls `stream.getTracks().forEach(stop)`, the `visibilitychange` handler, section-change, submit, unmount). Routing the recorder's `stream` state through the shared proctor stream meant those cleanups killed the live camera mid/after recording. So **video-response recording keeps its OWN `getUserMedia` stream** with the original stop-proctoring-on-record-start / restart-on-record-stop dance (Communication, Hinglish, Role-based); the reading-section voice and AI-interview answer also keep their own mic streams. Proctoring no longer freezes because of the DOM-mounted hidden `<video>` above (and it re-mounts cleanly when proctoring restarts after a recording). A true shared-stream would require unifying all those teardown sites first — not done.
