...
### Single shared camera stream — all assessment types (June 2026, UAT)

Root cause of "frozen" / byte-identical proctoring snapshots (one image repeated for a whole session — observed in PROD, 115 identical frames): the proctor capture `<video>` was created with `document.createElement` and **never inserted into the DOM**, and video-response recording **stopped the proctor stream to open its own camera**. WebKit/Safari (and backgrounded tabs on every engine) stop advancing frames for an offscreen/detached `<video>`, so `drawImage` silently re-copied the last painted frame. The earlier platform-wide sweep showed this is **not Safari-only** (Chrome/Blink also froze, ~6%), so the fix is engine-agnostic. Validated on iPhone/iOS Safari, then rolled out to **all** runtimes:
...
- **One stream for proctoring AND recording.** Proctoring opens a single **video+audio** `getUserMedia` stream, kept alive for the whole assessment. Video-response recording runs `MediaRecorder` on that **same** stream — the old "stop proctoring to release camera" teardown on record-start and the stop+restart on record-stop are **removed** (Communication, Hinglish, Role-based). Proctoring never pauses during/after a recording, and no second camera is opened.
- **Reading-section voice reuses the same mic.** `startAudioRecording` records a **clone of the shared stream's audio track** (`new MediaStream([track.clone()])`) instead of a second `getUserMedia({audio:true})` — avoids two concurrent mic captures (which glitched/silenced iOS voice recordings); the clone is independent, so the existing stop logic never kills the proctoring/video stream (Communication, Hinglish).
- **AI-interview** captures proctoring frames from the **existing visible self-view camera** (`selfVideoRef`) instead of opening a second camera — one `getUserMedia`, no two-camera contention.
