...
> **Diagnosis papers are excluded too (2026-09-16; DEV + UAT, PROD pending).** The same overwrite fired for a diagnosis pair when the student already had 2+ aptitude progression rows from elsewhere (another institute's chain, a corporate assign), so their diagnosis was served at their carried-in difficulty instead of the schedule's. The gate is now the shared `isAdaptiveDelivery({ isInstitute, isOneTime, isDiagnosis })` (`student-node/app/helpers/communicationSetSwap.js`), so one-time **and** `is_diagnosis = true` rows both serve exactly the assigned set. See the matching Communication note in `Assessment/communication.md`.
>
